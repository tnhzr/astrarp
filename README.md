...
**FlectonePulse** — формат чата с RP/NRP-индикатором и РП-именем:

```yaml
chat:
  format: "%luckperms_meta_astrarp_status% %luckperms_prefix%%luckperms_meta_astrarp_rpname% %message%"
```

**TAB** — добавьте RP-индикатор перед ником в табе:

```yaml
groups:
  default:
    tabprefix: "%luckperms_meta_astrarp_status% "
```
