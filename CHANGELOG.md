...
- **ChatHeads-интеграция полностью удалена.** Серверный суффикс,
  команда `/arp chatheads-aliases` и блок `chatheads:` в `config.yml`
  выпилены — на серверах с FlectonePulse + кастомными шрифтами
